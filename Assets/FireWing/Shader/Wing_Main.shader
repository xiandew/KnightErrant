...
////// Lighting:
////// Emissive:
                float4 node_4891 = _Time;
                float2 node_1412 = (i.uv0+node_4891.g*float2(-0.8,-0.8));
                float4 _Noise_var = tex2D(_Noise,TRANSFORM_TEX(node_1412, _Noise));
                float2 node_6021 = ((i.uv0+node_4891.g*float2(-0.3,-0.3))+_Noise_var.r*float2(0,0.4));
                float4 _SubTex_var = tex2D(_SubTex,TRANSFORM_TEX(node_6021, _SubTex));
                float2 node_4518 = (i.uv0+node_4891.g*float2(-0.3,-0.3));
                ...
                float3 emissive = (((_SubTex_var.rgb*_Transparency)*_Mask_var.a)*i.vertexColor.rgb*_TintColor.rgb*2.0);
